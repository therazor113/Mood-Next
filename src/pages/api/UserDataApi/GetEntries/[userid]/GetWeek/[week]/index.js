...
const getAllMoodsById = async (req, res) => {
  try {
    ...
    const moods = await pool.query(
      'SELECT weekday, json_agg(json_build_object(\'id\', id, \'mood\', mood, \'time\', time, \'date\', date, \'journal\', journal) ORDER BY time) AS entries FROM moods WHERE userid = $1 AND weekyear = $2 GROUP BY weekday ORDER BY weekday',
      [userid, week]
    )
